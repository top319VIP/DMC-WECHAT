...
class Http {
  ...
  constructor(){
    ...
    this.apiConfig = {"domainUrl":"http://127.0.0.1:8080/api/","mode": "cors","credentials":"include","headers":{"Content-Type":"application/json;charset=utf-8","Accept":"application/json"}};
  }
  ...
  setContentType(_string=''){
    ...
    this.apiConfig['headers']['Content-Type'] = _string;
  }
  ...
  setRequestHeader(_object){
    ...
    if(typeof _object == 'object'){
      if(_object instanceof Object){
        this.apiConfig['headers'] = Object.assign(this.apiConfig['headers'],_object);
        return this;
      }else{
  ...
  handle(_id,_method,_data,_callback){
    ...
    if(method === 'get'){
      url += this.transGetParams(_data);
      url = decodeURI(url);
      url = encodeURI(url);
      requestConfig["body"] = undefined;
    }else{
      requestConfig["body"] = JSON.stringify(_data);
    }
    
    return fetch(url,requestConfig).then(response=>{
      if(response['status'] === 200) return response.json();
      throw new Error('http response fail of json!');
    })
      .then(json=>{
        let resData = json;
        //if(response['status'] === 200) return response['json']
      ...
      .catch(function (error) {
        //todo 异常统一处理，暂时只抛出
          ...
          if (error.hasOwnProperty('response')&&typeof error.response === 'undefined') {
              ...
              let err = ""
              this.errors.message.forEach((v, i)=>{
